...
analytical ``_unnormalized_log_prob`` on its ``target_distribution``;
the algorithm-side decomposition is constructed by the user via
``DensityDecomposition.identity_from_target(problem.target_distribution)``
or another decomposition idiom.
"""
...
def banana(
    ...
    Args:
        d: parameter dimension. Must be ≥ 2.
        ...
        bounds: optional per-dim ``(lower, upper)`` overrides for the
            target's support. Defaults cover ~4σ in each dim.
        n_reference_samples: size of the analytic reference sample
            bank.

    Returns:
        `Problem` with ``input_shape=(d,)``, an analytical
        ``_unnormalized_log_prob`` on the ``target_distribution``, and
        a `NumericEmpiricalDistribution` reference built from exact
        factored draws. Construct a `DensityDecomposition` separately
        (``DensityDecomposition.identity_from_target(...)``).
    """
    if d < 2:
    ...
    # We use `independent_uniform` to derive the (d,)-box support.
